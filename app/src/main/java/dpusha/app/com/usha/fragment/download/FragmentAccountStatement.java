package dpusha.app.com.usha.fragment.download;

import android.os.Bundle;

import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import dpusha.app.com.usha.R;

public class FragmentAccountStatement extends Fragment {



    public FragmentAccountStatement() {
        // Required empty public constructor
    }


    public static FragmentAccountStatement newInstance(String param1, String param2) {
        FragmentAccountStatement fragment = new FragmentAccountStatement();
        Bundle args = new Bundle();

        fragment.setArguments(args);
        return fragment;
    }

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {

        }
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        return inflater.inflate(R.layout.fragment_account_statement, container, false);
    }
}